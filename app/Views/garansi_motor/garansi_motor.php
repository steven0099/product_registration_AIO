    ...
    <section class="content" style="margin-left: 290px; padding: 20px; margin-top: 80px;">
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title">Daftar Garansi Motor</h3>
                <button id="createmotorWarrantyBtn" class="btn btn-primary ml-auto">+ Tambah Garansi Motor</button>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="cowarTable" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    <?php $i = 1; ?>
                    <?php foreach ($garansi_motor as $motorwarranty): ?>
                      <tr>
                        <td><?= $i++; ?></td>
                        <td><?= esc($motorwarranty['value']) ?></td>
                        <td>
                          <button data-id="<?= esc($motorwarranty['id']) ?>" data-value="<?= esc($motorwarranty['value']) ?>" class="btn-edit btn btn-primary">
                            <i class="fas fa-pencil-alt"></i>
                          </button>
                          <button data-id="<?= esc($motorwarranty['id']) ?>" class="btn-delete btn btn-danger">
      ...
      <div id="myModal" class="modal">
        <div class="modal-content">
          ...
          <h3>Tambah Garansi Motor</h3>
          <form method="post" action="<?= base_url('/admin/garansi_motor/saveGaransiMotor') ?>" enctype="multipart/form-data">
          <?= csrf_field() ?> 
      ...
      <script>
        ...
        var addBtn = document.getElementById("createmotorWarrantyBtn");
        ...
        document.querySelectorAll('.btn-edit').forEach(function(button) {
          button.addEventListener('click', function() {
            ...
            document.getElementById('editForm').action = '/admin/garansi_motor/updateGaransiMotor/' + id;
        ...
        document.querySelectorAll('.btn-delete').forEach(function(button) {
          button.addEventListener('click', function() {
            ...
            document.getElementById('deleteForm').action = '/admin/garansi_motor/deleteGaransiMotor/' + id;
