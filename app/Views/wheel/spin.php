...
<body>
    ...
    function drawWheel() {
        ...
        segments.forEach(segment => {
            ...
            if (segment.imgElement) {
                const imageRadius = radius - 60; // Adjust distance from the center
                const imageAngle = currentAngle + segmentAngle / 2;

                const imgX = centerX + imageRadius * Math.cos(imageAngle) -
                25; // Adjust for half the image width
                const imgY = centerY + imageRadius * Math.sin(imageAngle) -
                25; // Adjust for half the image height
                ...
                ctx.beginPath();
                ctx.arc(imgX + 25, imgY + 25, 25, 0, 2 * Math.PI); // Circle clip path
                ctx.clip();

                // Draw the image
                ctx.drawImage(segment.imgElement, imgX, imgY, 50, 50);

                // Restore canvas state to avoid clipping other drawings
                ctx.restore();
            } else {
                console.warn(`No image for segment: ${segment.label}`);
            }
    ...
    function preloadImages(segments, callback) {
        ...
        if (totalImages === 0) {
            ...
            callback(); // No images to load; proceed to drawing
            return;
        }
        ...
        segments.forEach(segment => {
            if (segment.image) {
                const img = new Image();
                img.src = `/uploads/images/${segment.image}`; // Correct path to your images
                img.onload = () => {
                ...
                img.onerror = () => {
                    ...
                    loadedCount++; // Increment to avoid deadlock on error
                    if (loadedCount === totalImages) {
                        callback();
                    }
                };
            } else {
                loadedCount++; // Increment for segments without an image
                if (loadedCount === totalImages) {
                    callback();
                }
            }
    ...
    function loadSegments(newSegments) {
        segments = newSegments.map(segment => ({
            ...
            image: segment.image // Add the image file name
        }));
        ...
        function animate() {
            ...
            if (spinVelocity <= 0) {
                ...
                for (let segment of segments) {
                    const segmentAngle = (2 * Math.PI * segment.odds) / totalOdds; // Use odds for proportional segments
                    cumulativeAngle += segmentAngle;
